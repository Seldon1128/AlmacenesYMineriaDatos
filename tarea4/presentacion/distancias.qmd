...
```{python}
datos = np.array([casa_a, casa_b, casa_c], dtype=float)
...
En ellos, vemos que la diferencia de la casa A a la casa C parece abismal en comparación con la casa B, lo que es cierto si solo nos fijamos en el precio, pero **poco informativo si queremos tomar en cuenta que en el resto de los atributos las casas son muy similares**.
...
Por su naturaleza para el manejo de datos mixtos, para su cálculo es necesario saber el tipo de variable al que corresponde cada atributo y el rango de aquellas que son numéricas, por lo que además de los dos objetos a evaluar, es necesaria una lista en la que se especifica por cada catributo, si es categórico o numérico y otra lista con los rangos de cada atributo (0 si es categórica).
